...
export default defineEventHandler((event) => {
    ...
    // Get both en and zh tranlation
    try{
        const enFilePath = path.join(process.cwd(), `locale/${type}`, `${String(name).replace(".json", "_en.json")}`)
        const zhFilePath = path.join(process.cwd(), `locale/${type}`, `${String(name).replace(".json", "_zh.json")}`)
